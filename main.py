...
app.config['SQLALCHEMY_DATABASE_URI']='postgresql://postgres:theo@localhost:5432/postgres'
...
def processData():
    ...
    if request.method=="POST":
        ...
        if database =="postgres":
            ...
            try:
                conn = mariadb.connect(user="root",password=" ",host="127.0.0.1",port=3306,database="maria")
...
def retrieveData(database):
    if database == 1:
        ...
        conn = pyodbc.connect("Driver={ODBC Driver 17 for SQL Server};""Server=AQUILA-KPASOYA;" "Database=microsoft;""Trusted_Connection=yes;")
        cursor = conn.cursor()
        cursor.execute("SELECT * FROM  microsoft.dbo.person")
